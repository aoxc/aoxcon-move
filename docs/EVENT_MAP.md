...
- `CommandApplied { command_id, digest, payload_kind, quorum_signers, pause_state_after, executed_at_ms }`
...
- `RewardDistributed { user, amount }`
  ...
  - Index keys: `user`.
- `MerkleRootPublished { epoch, root_hash }`
  - Trigger: scalable claim mode root publication.
- `MerkleRewardClaimed { epoch, user, amount }`
  - Trigger: successful claim-mode payout.
...
## Module: `staking`
- `StakeAdded { user, amount }`
- `Unstaked { user, amount }`
- `Slashed { amount, slash_bps }`
- `AutoCompounded { amount, compounded_total }`

## Module: `liquidity_manager`
- `SwapRouted { dex, amount_in, min_out }`
- `LiquidityRouted { dex, amount_a, amount_b }`

## Module: `marketplace`
- `DatasetListed { id, seller, ask_amount }`
- `DatasetPurchased { id, buyer, ask_amount }`
- `DatasetCancelled { id, seller }`
