...
11. `staking::StakeAdded`
12. `staking::Slashed`
13. `staking::AutoCompounded`
14. `liquidity_manager::SwapRouted`
15. `liquidity_manager::LiquidityRouted`
16. `marketplace::DatasetListed`
17. `marketplace::DatasetPurchased`
18. `marketplace::DatasetCancelled`
...
## 8. Brand Manifest Monitoring
- Track manifest version changes for AOXC and reputation display profiles.
- Alert on checksum changes without corresponding governance action timeline.
- Persist release-hash audit trail for every brand update.

## 9. SLO/SLI Suggestions
- **SLI:** % of `ProfileUpdated` events with valid relay-anchor link.
- **SLI:** % of Merkle claims passing off-chain recomputation.
- **SLO:** 99.9% reconciliation success over rolling 7 days.
