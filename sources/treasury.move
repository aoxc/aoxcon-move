module aoxc::treasury {
    ...
    public fun validate_yield_policy(
        lending_enabled: bool,
        liquidity_enabled: bool,
        lending_allocation_bps: u16,
        liquidity_allocation_bps: u16,
        lending_adapter_len: u64,
        liquidity_adapter_len: u64,
    ) {
        assert!((lending_allocation_bps as u64) + (liquidity_allocation_bps as u64) <= 10_000, errors::E_POLICY_LIMIT);
        assert!(lending_enabled || liquidity_enabled, errors::E_POOL_NOT_ENABLED);
        if (lending_enabled) { assert!(lending_adapter_len > 0, errors::E_INVALID_ARGUMENT); };
        if (liquidity_enabled) { assert!(liquidity_adapter_len > 0, errors::E_INVALID_ARGUMENT); };
    }
    ...
    entry fun set_yield_hooks(
        ...
        validate_yield_policy(lending_enabled, liquidity_enabled, lending_allocation_bps, liquidity_allocation_bps, vector::length(&lending_adapter), vector::length(&liquidity_adapter));
    ...
    entry fun rebalance_yield<T>(
        ...
        circuit_breaker::assert_live(breaker);
        assert!(hooks.lending_enabled || hooks.liquidity_enabled, errors::E_POOL_NOT_ENABLED);
