module aoxc::sentinel_dao {
    ...
    const ACTION_PAUSE_PROTOCOL: u8 = 1;
    const ACTION_RESUME_PROTOCOL: u8 = 2;
    ...
    /// Queue AI/Sentinel decision and enforce delay before execution.
    entry fun queue_proposal(
        ...
        let valid = action_type == ACTION_PAUSE_PROTOCOL || action_type == ACTION_RESUME_PROTOCOL;
        ...
        event::emit(ProposalQueued { id, action_type, eta_ms });
    }
    ...
    entry fun finalize_proposal(
        ...
        if (p.action_type == ACTION_PAUSE_PROTOCOL) { dao.paused = true; };
        if (p.action_type == ACTION_RESUME_PROTOCOL) { dao.paused = false; };
